...
export const deleteNotification = async (req, res) => {
    ...
    try {
        if (!req.body._id) throw new Error("Emp_id required")
        if (!req.body.attendance_id) throw new Error("Attendance  required")

        const data = await VerifyActivity.findOne({ emp_id: req.body._id, attendance_id: req.body.attendance_id, status: 0 })
...
export const verifyActivities = async (req, res) => {
    ...
    try {
        ...
        if (!req.body.attendance_id) throw Error("No Attendance Id provided!")
        if (!req.body.admin_id) throw Error("No Admin Id provided!")
        // if (!req.body.subject) throw Error("No Subject Id provided!")
        if (!req.body.message) throw Error("No Message Id provided!")

        const chekIfAreadyResponded = await VerifyActivity.findOne({ attendance_id: req.body.attendance_id })
        if (chekIfAreadyResponded)
            throw Error("Already Responded")

        const attendanceCheck = await Attendance.findOne({ _id: req.body.attendance_id })
        const empID = await Employee.findOne({ emp_id: attendanceCheck.emp_id })

        if (!attendanceCheck)
            throw Error("Attendance not present")
...
export const getVerifiedActivity = async (req, res) => {
    ...
    try {
        ...
        const data = await VerifyActivity.find({ admin_id: req.body.admin_id })
            .populate({ path: "emp_id", select: ['name', 'email', 'phone'] })
