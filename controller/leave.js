...
export const applyCasualLeave = async (req, res) => {
    ...
    try {
        ...
        console.log("Available", availablePaidLeaves);
        ...
        const leave = new Leave({
            emp_id: emp_id,
            empObject_id: req.body.empObject_id,
...
// get leave history without pages with status by HR
export const getLeaveRequest = async (req, res) => {
    const out = {}
    try {
        if (parseInt(!req.body.status)) throw Error("No status applied")

        const leaveCheck = await Leave.find({ status: req.body.status })
            .populate({ path: "empObject_id", select: ['name', 'email', 'designation'] })
            .sort({ applied_on: -1 })

        // let emp = []
        // for (var i = 0; i < leaveCheck.length; i++) {
        //     const result = await Employee.findOne({emp_id: leaveCheck[i].emp_id})
...
// get leave history without pages with status by Admin
export const getLeaveRequestByAdmin = async (req, res) => {
    const out = {}
    try {
        if (parseInt(!req.body.status)) throw Error("No status applied")

        const leaveCheck = await Leave.find({ status: req.body.status, hrStatus: 1 })
            .populate({ path: "empObject_id", select: ['name', 'email', 'designation'] })
            .sort({ applied_on: -1 })
...
export const getDetailByID = async (req, res) => {
    ...
    try {
        ...
        const leaveCheck = await Leave.findOne({ _id: req.body._id })
...
//hr staus Change using
export const statusChangeByHR = async (req, res) => {
    const out = {}
    try {
        ...
        if (!req.body.hrStatus)
            throw Error("status is required")

        const hrStatus = parseInt(req.body.hrStatus)
        let status = ""
        if (hrStatus === 2) // rejected
        {
            if (!req.body.hrRemark)
        ...
        const update = {
            hrStatus: hrStatus,
            hrRemark: req.body.hrRemark,
            status: status,
            hr_status_modified_date: getformattedDate()
        }
...
//Admin staus Change using
export const statusChangeByAdmin = async (req, res) => {
    const out = {}
    try {
        const id = req.body._id
        if (!req.body._id)
