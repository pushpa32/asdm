...
async function saveEmp(data) {
  ...
  const emp = new Employee({
    ...
    attached_with: data.emp.attached_with,
    project: data.emp.project,
    first_district: data.emp.first_district,
    additional_district: data.emp.additional_district,
  });
...
export const importEmp = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < temp.length; i++) {
      ...
      emp.phone = res["Contact"];
      emp.email = res["Email"];
      emp.attached_with = res["Attached with"];
      emp.project = res["Project"];
      emp.first_district = res["1ST DISTRICT"];
      emp.additional_district = res["ADDITIONAL DISTRICT"];
